...
/*
	...
	Plugin URI: http://www.q2apro.com/plugins/comment-to-answer
...
*/
	...
	class q2apro_comment_to_answer_page {
		...
		function process_request($request)
		{
			...
			// return if not admin or moderator level
			$level = qa_get_logged_in_level();
			$userid = qa_get_logged_in_userid();
			if ($level<QA_USER_LEVEL_MODERATOR) {
				// allowed: Kai, Unknown, Lu
				if($userid!=637 && $userid!=134 && $userid!=167) 
				{
			...
			if(isset($commentid)) {
				...
				// commentid can be ID or LINK
				if(strpos($commentid,'http') !== false) {
					$posstart = strpos($commentid, '#c')+2;
					$strlength = strlen($commentid)-$posstart;
					$commentid = substr($commentid,$posstart,$strlength);
					if((int)$commentid != $commentid) {
				...
				if($typeCheck!='C') {
					...
					$qa_content['custom'] .= '<p>'.qa_lang('q2apro_comment_to_answer_lang/error0').' '.$wrongid_link.'</p>';
					$qa_content['custom'] .= '<a href="./convertcomment" class="btnblue">'.qa_lang('q2apro_comment_to_answer_lang/return').'</a>';
					return $qa_content;
				}
				...
				if($parenttype=='Q') {
					...
					// content output error
					$qa_content['custom'] .= '<p>'.qa_lang('q2apro_comment_to_answer_lang/error3').'</p>';
					$qa_content['custom'] .= '<a href="./convertcomment" class="btnblue">'.qa_lang('q2apro_comment_to_answer_lang/return').'</a>';
					return $qa_content;
				}
				...
				if(isset($questionid) && isset($created)) {
					...
					if(isset($newAcount)) {
						...
						$qa_content['custom'] .= '<p>'.qa_lang('q2apro_comment_to_answer_lang/success').' → <a target="_blank" href="'.qa_opt('site_url').$questionid.'?show='.$commentid.'#a'.$commentid.'">ID: '.$commentid.'</a></p>';
						$qa_content['custom'] .= '<a href="./convertcomment" class="btnblue">'.qa_lang('q2apro_comment_to_answer_lang/return').'</a>';
						return $qa_content;
					}
					else {
						// content output error
						$qa_content['custom'] .= '<p>'.qa_lang('q2apro_comment_to_answer_lang/error1').'</p>';
						$qa_content['custom'] .= '<a href="./convertcomment" class="btnblue">'.qa_lang('q2apro_comment_to_answer_lang/return').'</a>';
						return $qa_content;
					}
				}
					...
					// content output error
					$qa_content['custom'] .= '<p>'.qa_lang('q2apro_comment_to_answer_lang/error2').'</p>';
					$qa_content['custom'] .= '<a href="./convertcomment" class="btnblue">'.qa_lang('q2apro_comment_to_answer_lang/return').'</a>';
					return $qa_content;
				}
			}
			...
			$qa_content['custom'] = '';
			$qa_content['custom'] .= '<p>Die Kommentar-ID sieht man, wenn man mit der Maus über "Kommentiert von" fährt. Es ist die Zahl am Ende der URL.</p>';
			$qa_content['custom'] .= '<p>Vorgehen:<br />
										1. Klick auf "Kommentiert von"<br />
										2. Kopiere den Link oben aus der Browser-Adressleiste<br />
										3. Füge ihn hier ein.</p>';
			$qa_content['custom'] .= '<div id="convdiv">
											<form name="uploadform" method="post" action="'.qa_self_html().'">
												<input name="commentid" id="commentid" type="text" placeholder="'.qa_lang('q2apro_comment_to_answer_lang/input_placeholder').'">
												<br />
			...
			$qa_content['custom'] .= '<p style="margin-top:100px;color:#C00;">Achtung: Verschiebungen sind nicht ohne Weiteres rückgängig zu machen.</p>';
			$qa_content['custom'] .= '
			<style type="text/css">
			.qa-main {
				line-height:170%;
